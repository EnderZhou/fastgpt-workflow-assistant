...
Copy-Item dist/fastgpt-workflow-assistant-v1.3.0.zip `
  dist/fastgpt-workflow-assistant-latest.zip -Force
Get-FileHash dist/fastgpt-workflow-assistant-v1.3.0.zip -Algorithm SHA256
```
...
python skill/fastgpt-workflow-assistant/scripts/check_skill_version.py `
  ...
  --package dist/fastgpt-workflow-assistant-v1.3.0.zip `
  --json
```

提交版本文件后创建带注释的 Tag `v1.3.0` 和 GitHub Release。上传以下三个附件：

- `fastgpt-workflow-assistant-v1.3.0.zip`：不可变版本包，用于审计与回滚；
